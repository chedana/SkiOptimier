...
- **航班自动抓取** — Playwright 无头浏览器自动打开 Google Flights，提取所有直飞航班（航班号、时间、价格、行李），替代不可靠的 LLM 浏览器操作
- **航班排序格式化** — 确定性排序引擎，支持中文偏好（早到、晚回、最便宜、不要红眼、全服务），输出 Discord 格式化列表
- **独立航班技能** — `flight_search` 模块可独立使用，不依赖滑雪路线规划。任何航班查询都可触发
- **价格搜索 CLI** — `ski-run.sh price-search` 为每段行程生成搜索查询和 URL
- **Web UI** — 响应式网页界面，浏览路线、查看详情、选择路线（移动端适配）
- **日期选择器** — Web UI 支持出发/返回日期选择，日期随路线一起提交
- **Discord 代码块表格** — 航班信息用等宽代码块表格展示，价格/时间/行李对齐清晰
...
├── web/                        # React + Vite 前端
│   ├── src/
│   │   ├── App.jsx             # 主应用（购物车、路线卡片、日期选择器）
│   │   ├── routes-cache.js     # 预生成路线缓存
│   │   └── main.jsx
│   ├── api/
│   │   └── confirm.js          # Vercel serverless — Discord webhook（含日期）
│   ├── index.html
│   └── package.json
├── flight_search/              # 航班搜索模块（独立）
│   ├── cli.py                  # CLI 入口（scrape-flights/format-flights）
│   ├── scrape.py               # Playwright Google Flights 抓取器
│   └── formatter.py            # 航班排序 + Discord 格式化
├── skills/
│   ├── ski/
│   │   └── SKILL.md            # 滑雪路线规划技能
│   ├── ski-ticket/
│   │   └── SKILL.md            # 雪票/装备租赁技能
│   └── flight/
│       └── SKILL.md            # 通用航班搜索技能（OpenClaw config 中）
├── ski_planner/                # 滑雪路线 CLI 工具
│   ├── cli.py                  # CLI 入口（search/details/price-search）
│   ├── price_search.py         # 价格搜索查询生成器
│   ├── travel_sites.json       # 机场/车站/巴士 ID 映射
│   └── ticket.py               # 雪票查询
├── ski-run.sh                  # 路线搜索入口脚本
├── flight-run.sh               # 航班搜索入口脚本（Docker 容器中）
...
独立模块，可单独使用，不依赖滑雪路线规划。

### CLI 命令

```bash
# 抓取 Google Flights 直飞航班（Playwright 无头浏览器）
python -m flight_search.cli scrape-flights "London" "Venice" "2026-03-29" --timeout 60

# 排序 + 格式化（输出 Discord 格式）
python -m flight_search.cli format-flights '{"direction":"outbound","date":"3/29","origin":"London","destination":"Venice","preference":"早到","flights":[...]}'
```
...
```bash
# 容器中调用
/home/node/.openclaw/workspace/flight-run.sh scrape-flights "London" "Paris" "2026-03-29" --timeout 60
```

需要 Playwright Chromium 已安装（`OPENCLAW_INSTALL_BROWSER=1`）。
...
- **航班抓取**：Playwright (headless Chromium)
